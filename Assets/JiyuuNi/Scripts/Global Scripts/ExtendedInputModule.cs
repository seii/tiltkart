...
{
    [AddComponentMenu("Event/Extended Input Module")]
    /// <summary>
    /// A BaseInputModule designed for mouse / keyboard / controller input.
    /// Also works with TiltFive Wand controller in both Wand Mode and Gamepad Mode.
    /// </summary>
    /// <remarks>
    /// Input module for working with, mouse, keyboard, or controller.
    /// Also works with TiltFive Wand controller in Wand Mode and Gamepad Mode.
    /// </remarks>
    public class ExtendedInputModule : PointerInputModule
    {
        ...
        private bool t5TriggerPressed = false;
        ...
        public TiltFiveProperties.T5ControllerMode lastT5Orientation =
            TiltFiveProperties.T5ControllerMode.Wand;
        ...
        public override bool ShouldActivateModule()
        {
            ...
            shouldActivate |= TiltFive.Input.GetButtonUp(TiltFive.Input.WandButton.A);
            shouldActivate |= TiltFive.Input.GetTrigger() > 0.3f ? true : false;
            shouldActivate |= !Mathf.Approximately(TiltFive.Input.GetStickTilt().x, 0.0f);
            shouldActivate |= !Mathf.Approximately(TiltFive.Input.GetStickTilt().y, 0.0f);
        ...
        protected bool SendSubmitEventToSelectedObject()
        {
            ...
            //TODO: Does this need to be created with any special values for T5?
            ...
            // Trigger is a stream, make sure it only sends one event per press
            if (TiltFive.Input.GetTrigger() > 0.3f)
            {
                if(!t5TriggerPressed)
                {
                    ExecuteEvents.Execute(eventSystem.currentSelectedGameObject, data, ExecuteEvents.submitHandler);
                    t5TriggerPressed = true;
                }
            }else
            {
                if(t5TriggerPressed)
                {
                    t5TriggerPressed = false;
                }
            }

            if (TiltFive.Input.GetButtonUp(TiltFive.Input.WandButton.One))
                ExecuteEvents.Execute(eventSystem.currentSelectedGameObject, data, ExecuteEvents.cancelHandler);
        ...
        private TiltFiveProperties.T5ControllerMode GetT5ControllerOrientation(Vector3 currentRotation)
        {
            ...
            if (TiltFive.Input.GetWandAvailability() && TiltFive.Wand.IsTracked())
            {
                ...
                //print($"ExtendedInputModule: {lastT5Orientation}");
            }

            return result;
        }

        private void GetCoordinatesFromWand(Vector3 stickCoordinates, out float finalX, out float finalY)
        {
            Vector3 currentRotation = TiltFive.Wand.GetRotation(TiltFive.ControllerIndex.Right).eulerAngles;
            TiltFiveProperties.T5ControllerMode mode = GetT5ControllerOrientation(currentRotation);

            if(mode.Equals(TiltFiveProperties.T5ControllerMode.GamepadLeft))
            {
                // Gamepad, pointing left
                //print($"ExtendedInputModule: gamepad left");
                finalX = -1 * stickCoordinates.y;
                finalY = stickCoordinates.x;
            }
            else if(mode.Equals(TiltFiveProperties.T5ControllerMode.GamepadRight))
            {
                // Gamepad, pointing right
                //print($"ExtendedInputModule: gamepad right");
                finalX = stickCoordinates.y;
                finalY = -1 * stickCoordinates.x;
            }
            else
            {
                // Wand
                //print($"ExtendedInputModule: wand");
                finalX = stickCoordinates.x;
                finalY = stickCoordinates.y;
            }
        }
        ...
        private Vector2 GetRawMoveVector()
        {
            ...
            Vector2 moveT5 = Vector2.zero;
            Vector2 moveStd = Vector2.zero;

            if(TiltFive.Input.GetWandAvailability())
            {
                GetCoordinatesFromWand(TiltFive.Input.GetStickTilt(),
                    out moveT5.x, out moveT5.y);
            }

            moveStd.x = input.GetAxisRaw(m_HorizontalAxis);
            moveStd.y = input.GetAxisRaw(m_VerticalAxis);

            // Uses whichever value is greater for input
            //   This allows other input systems to be used even when the
            //   Tilt Five wand is connected.
            move.x = Mathf.Abs(moveT5.x) > Mathf.Abs(moveStd.x) ? moveT5.x : moveStd.x;
            move.y = Mathf.Abs(moveT5.y) > Mathf.Abs(moveStd.y) ? moveT5.y : moveStd.y;
